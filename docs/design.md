...
| `china` | 国内要闻 | 🏠 | Google News (中国), 人民日报, 央视新闻 |
| `tech` | 科技新闻 | 💻 | TechCrunch, The Verge, 36氪 |
| `business` | 商业新闻 | 📈 | Bloomberg, The Information（按内容路由） |
| `ai` | AI | 🤖 | TechCrunch AI, The Verge AI, MIT Technology Review |
...
```
1. 读取 config/categories.json，获取所有新闻源配置
2. 根据 `source.type` 抓取 RSS Feed 或 News Sitemap
3. 解析条目，提取标题、链接、图片、原始摘要
4. 对支持内容路由的来源按标题/链接规则映射到 ai / tech / business
5. 基于链接 hash 去重（对比当天已有数据）
6. 调用火山引擎大模型 API：
   - 英文新闻 → 翻译标题 + 生成中文总结
   - 中文新闻 → 生成精炼总结
   - 仅有标题的来源 → 生成保守标题型摘要
7. 组装 JSON 数据，先保留原始 `articles`
8. 对同一期次新闻做主题聚合，生成 `clusters`
9. 写入 data/YYYY-MM-DD[-edition].json
10. 更新 data/index.json
11. Git commit + push
```
